...
export function AvgBatteryCard() {
	const { avgBattery, avgBatteryLoading, avgBatteryError } = useSocketStore()
	return (
		<div className='dashboard-card !h-full'>
			{avgBatteryLoading ? (
				<div className='spinner-load-container dashboard-card-load-font'>
					<Spinner className='size-4 m-1' /> считаем заряд батарей...
				</div>
			) : avgBatteryError ? (
				<div className='spinner-load-container dashboard-card-load-font'>
					{avgBatteryError}
				...
				>
					<h3 className='dashboard-section-font'>Ср. заряд батарей</h3>
